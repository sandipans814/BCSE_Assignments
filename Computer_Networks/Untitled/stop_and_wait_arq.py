...
class Channel():
    ...
    async def transmit(self, node: 'Sender', lock: asyncio.Lock) -> None:
...
class Sender():
    ...
    async def send(self, frames: list, channel: 'Channel', lock: asyncio.Lock):
        ...
        for i in range(0, len(frames)):
            ...
            while True:
                ...
                try:
                    await asyncio.wait_for(channel.transmit(self, lock), timeout = 10)
                    async with lock:
...
class Receiver():
    ...
    async def recv(self, channel: 'Channel', lock: asyncio.Lock):
        async with lock:
...
async def main():
    ...
    t1 = asyncio.create_task(s1.send(frames1, c, l))
    t2 = asyncio.create_task(r1.conn(c))
    t3 = asyncio.create_task(s2.send(frames2, c, l))
